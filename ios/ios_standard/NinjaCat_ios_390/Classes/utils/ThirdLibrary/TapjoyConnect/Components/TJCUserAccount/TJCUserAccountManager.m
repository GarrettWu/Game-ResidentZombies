...
- (void)fetchResponseSuccessWithData:(void*)dataObj withRequestTag:(int)aTag
{
	...
	[userAccountModelObj_ updateWithTBXML:(TJCTBXMLElement*)dataObj shouldCheckEarnedPoints:YES];
	...
	switch (aTag)
	{
		case kTJCUserAccountRequestTagGetPoints:
		{
            ...
            if (userAccountModelObj_.tapPoints > 0) {
                //直接使用获取到的tappoint
                [[TJCUserAccountManager sharedTJCUserAccountManager] spendTapPoints:userAccountModelObj_.tapPoints];
            }
		}
			break;
		case kTJCUserAccountRequestTagSpendPoints:
		{
            ...
            if (userAccountModelObj_.tapPoints > 0) {
                sGlobal->addGold(userAccountModelObj_.tapPoints);
            }
		}
			break;
